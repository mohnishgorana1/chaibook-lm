                          ...
                          <div className="mt-2 flex flex-wrap gap-2">
                            {currentCitations.map((cite, i) => (
                              ...
                              >
                                ...
                                <span className="max-w-[120px] truncate" title={cite.title}>{cite.title || `Source ${i + 1}`}</span>
          ...
          {!activePreview ? (
            <div className="flex h-full flex-col items-center justify-center text-center text-muted">
              <div className="w-full max-w-[280px] rounded-[24px] border border-dashed border-subtle p-8 bg-base/50">
                ...
                <p className="text-[13px] leading-relaxed">Click a citation source<br />to preview exact timestamp or page.</p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-4 animate-in fade-in duration-300">
              {((activePreview.type === "YOUTUBE" && activePreview.timestamp !== undefined) || (activePreview.type === "PDF" && activePreview.pageNumber !== undefined)) && (
                <div className="flex items-center gap-2 rounded-lg bg-emerald-500/10 px-3 py-2 border border-emerald-500/20">
                  <Sparkles className="h-4 w-4 text-emerald-500" />
                  <span className="text-[12px] font-medium text-emerald-600 dark:text-emerald-400">
                    {activePreview.type === "YOUTUBE" ? `Deep-linked to ${formatTimestamp(activePreview.timestamp || 0)}` : `Deep-linked to Page ${activePreview.pageNumber}`}
                  </span>
                </div>
              )}
              ...
              {(activePreview.type === "YOUTUBE" || activePreview.type === "PDF") && activePreview.contentUrl && (
                <div className="overflow-hidden rounded-2xl border border-subtle bg-base shadow-sm">
                  ...
                  {activePreview.type === "YOUTUBE" ? (
                    <div className="aspect-video w-full bg-black">
                      <iframe className="h-full w-full" src={`${activePreview.contentUrl.replace("watch?v=", "embed/")}${activePreview.timestamp ? `?start=${activePreview.timestamp}&autoplay=1` : ""}`} allowFullScreen />
                    </div>
                  ) : (
                    <div className="aspect-[1/1.4] w-full bg-input">
                      <iframe src={`${activePreview.contentUrl}#view=FitH${activePreview.pageNumber ? `&page=${activePreview.pageNumber}` : ""}`} className="h-full w-full opacity-90" />
                    </div>
                  )}
                </div>
              )}
